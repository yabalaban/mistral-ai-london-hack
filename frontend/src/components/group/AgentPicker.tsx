...
export function AgentPicker({ onSelect, onClose, excludeIds = [] }: AgentPickerProps) {
  ...
  return (
    ...
    >
      <div class="bg-dc-secondary rounded-lg max-w-sm w-full mx-4 p-4">
        <h3 class="text-lg font-semibold text-dc-text mb-3">Add Agents</h3>
            ...
            <button
              ...
              class={`w-full flex items-center gap-3 px-3 py-2 rounded transition-colors ${
                selected.has(agent.id)
                  ? 'bg-dc-blurple/20 text-dc-text'
                  : 'hover:bg-dc-tertiary text-dc-text-secondary'
              }`}
            >
              ...
              {selected.has(agent.id) && (
                <svg class="w-5 h-5 ml-auto text-dc-blurple flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                  <path
                    fill-rule="evenodd"
